...
export const FilesTab: React.FC<FilesTabProps> = ({
  ...
  const countMatches = (text: string, highlight: string): number => {
  ...
  const currentFileIssue = selectedFile ? getSecurityIssueForFile(selectedFile) : null;
  ...
  const originalContent = selectedFile ? getFileContent(selectedFile) : "";
  const securedContent = (currentFileIssue && currentFileIssue.oldCode && currentFileIssue.newCode)
    ? originalContent.replace(currentFileIssue.oldCode, currentFileIssue.newCode)
    : originalContent;
