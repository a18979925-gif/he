...
export const DatabaseTab: React.FC<DatabaseTabProps> = ({
  ...
  const getDataTypeBadgeStyle = (type: string) => {
    ...
    if (t.includes("varchar") || t.includes("text") || t.includes("string") || t.includes("char")) {
      return "text-purple-700 dark:text-purple-300 bg-purple-50 dark:bg-purple-950/40 border border-purple-200/50 dark:border-purple-800/40";
    }
    if (t.includes("int") || t.includes("number") || t.includes("float") || t.includes("decimal") || t.includes("double") || t.includes("numeric") || t.includes("serial") || t.includes("bigint")) {
      return "text-cyan-700 dark:text-cyan-305 bg-cyan-50 dark:bg-cyan-950/40 border border-cyan-200/50 dark:border-cyan-800/40";
    }
    if (t.includes("bool")) {
      return "text-amber-700 dark:text-amber-305 bg-amber-50 dark:bg-amber-950/40 border border-amber-200/50 dark:border-amber-800/40";
    }
    if (t.includes("date") || t.includes("time") || t.includes("stamp")) {
      return "text-emerald-700 dark:text-emerald-305 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200/50 dark:border-emerald-800/40";
    }
    return "text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-805 border border-slate-200/50 dark:border-slate-700/40";
  };

  // Helper: Constraints badge parser
  const renderConstraintBadges = (constraints?: string) => {
    if (!constraints) return <span className="text-slate-400 dark:text-slate-600 font-sans">-</span>;
      ...
      <div className="flex flex-wrap gap-1">
        {badges.map((badge, idx) => {
          ...
          if (badge === "PRIMARY KEY") {
            style = "bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/25";
          } else if (badge === "FOREIGN KEY") {
            style = "bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border border-indigo-500/25";
          } else if (badge === "NOT NULL") {
            style = "bg-rose-500/5 text-rose-600 dark:text-rose-400 border border-rose-500/20";
          } else if (badge === "UNIQUE") {
            style = "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/25";
          }
          return (
            <span key={idx} className={`text-[10px] font-mono px-2 py-0.5 rounded font-semibold ${style}`}>
              {badge}
            </span>
          );
        })}
  ...
  const ERDTableCard = ({ table, isSelected, role }: { table: DBTable, isSelected: boolean, role: 'source' | 'target' | 'selected', key?: any }) => {
    ...
    return (
      <div 
        ...
        className={`rounded-2xl border transition-all duration-300 text-left bg-slate-900/90 backdrop-blur w-full shadow-xl ${
          isSelected 
            ? "border-indigo-500 shadow-[0_0_30px_rgba(99,102,241,0.25)] ring-1 ring-indigo-500/50" 
            : "border-slate-800/80 hover:border-slate-700 cursor-pointer hover:shadow-indigo-950/20 hover:scale-[1.01]"
        }`}
      >
        {/* Card Header */}
        <div 
          ...
          className={`px-4 py-3 border-b rounded-t-2xl flex items-center justify-between ${
            isSelected 
              ? "bg-gradient-to-r from-indigo-950 to-indigo-900/40 border-indigo-500/30" 
              : "bg-slate-950/80 border-slate-800/60"
          }`}
        >
          <div className="flex items-center gap-2.5 overflow-hidden">
            <Database className={`h-4 w-4 shrink-0 ${isSelected ? "text-indigo-400" : "text-slate-500"}`} />
            <span className="text-xs font-mono font-bold text-slate-100 truncate" title={table.name}>
              {table.name}
            </span>
          </div>
          {role !== 'selected' && (
            <span className="text-[9px] bg-slate-800 text-slate-400 px-2 py-0.5 rounded font-mono font-medium shrink-0">
          ...
          {(isSelected ? table.columns : visible).map((col, idx) => {
            ...
            return (
              <div 
                ...
                className="flex items-center justify-between px-4 py-2 hover:bg-slate-800/35 transition-colors text-[10px] font-mono group"
              >
                <div className="flex items-center gap-2 overflow-hidden">
                  ...
                  <span className="text-slate-300 font-bold truncate group-hover:text-white">
                    {col.name}
                  </span>
                </div>
                <span className="text-slate-500 text-[9px] font-mono ml-2 shrink-0 truncate max-w-[85px]">
                  {col.type}
                </span>
              </div>
            );
        ...
        {/* Hidden count badge */}
        {role !== 'selected' && hiddenCount > 0 && (
          <div className="px-4 py-1.5 text-[9px] text-slate-500 bg-slate-950/20 text-center font-mono border-t border-slate-800/30 rounded-b-2xl">
  ...
  return (
    <div className="space-y-6 text-left" id="database-tab-view">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .erd-flow-line {
          ...
          animation: erdDash 1.2s linear infinite;
        }
      `}} />

      {/* Header and Stats */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5" id="database-tab-header">
        <div className="text-left">
          <div className="flex items-center gap-3">
            <span className="p-2.5 bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 rounded-2xl shadow-sm border border-indigo-100 dark:border-indigo-900/30">
              <Database className="h-5 w-5" />
            </span>
            <div>
              <h2 className="text-lg font-bold tracking-tight text-slate-900 dark:text-slate-55">
                Database Schema Reverse-Engineering
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Prisma / Hibernate code structures parsed into an interactive relational Entity-Relationship Model (ERD).
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-2 rounded-2xl shadow-sm text-left min-w-[70px]">
            <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-bold block tracking-wider">Tables</span>
            <span className="text-sm font-extrabold font-mono text-slate-800 dark:text-indigo-400">
              {totalTables}
            </span>
          </div>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-2 rounded-2xl shadow-sm text-left min-w-[70px]">
            <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-bold block tracking-wider">Relations</span>
            <span className="text-sm font-extrabold font-mono text-slate-800 dark:text-pink-400">
              {totalRelations}
            </span>
          </div>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-2 rounded-2xl shadow-sm text-left min-w-[70px]">
            <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-bold block tracking-wider">Columns</span>
            <span className="text-sm font-extrabold font-mono text-slate-800 dark:text-emerald-400">
              {totalColumns}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Tables list on left */}
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 lg:col-span-1 flex flex-col h-full min-h-[500px]">
          <div>
            <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block mb-3 text-left">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/50 text-slate-700 dark:text-slate-250 transition-all font-sans"
              />
            </div>
          </div>

          <div className="space-y-1.5 flex-1 overflow-y-auto max-h-[600px] pr-1">
            {filteredTables.length === 0 ? (
              <div className="p-8 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
                <Database className="h-6 w-6 text-slate-400 dark:text-slate-600 mx-auto mb-2 animate-pulse" />
                <p className="text-xs text-slate-500 dark:text-slate-400">No tables match search</p>
              </div>
            ) : (
                ...
                <button
                  ...
                  className={`w-full text-left p-3.5 rounded-xl border transition-all duration-200 flex items-center justify-between cursor-pointer group ${
                    selectedTable?.name === table.name
                      ...
                      : "bg-slate-50/50 dark:bg-slate-800/10 hover:bg-slate-100 dark:hover:bg-slate-800/30 border-slate-200 dark:border-slate-800/60"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className={`text-xs font-mono font-bold transition-colors ${
                        selectedTable?.name === table.name 
                          ? "text-slate-900 dark:text-indigo-200" 
                          : "text-slate-700 dark:text-slate-300 group-hover:text-slate-950 dark:group-hover:text-white"
                      }`}>
                        {table.name}
                      </span>
                      <span className="text-[9px] text-slate-400 dark:text-slate-500 font-sans mt-0.5">
        ...
        {selectedTable ? (
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-3.5">
              ...
              {/* ERD Canvas Wrapper */}
              <div className="overflow-x-auto overflow-y-hidden border border-slate-200 dark:border-slate-800/80 rounded-2xl bg-slate-950">
                <div 
                  ref={canvasRef}
                  className="relative p-6 min-h-[385px] flex items-center justify-between select-none min-w-[760px] gap-12"
                  style={{
                ...
                >
                      ...
                      return (
                        <g key={conn.id} className="group/line">
                          ...
                          <path 
                            ...
                            stroke={gradUrl} 
                            strokeWidth={1.5} 
                            className="erd-flow-line opacity-80 group-hover/line:opacity-100 group-hover/line:stroke-[2px] transition-all"
                          />
                        </g>
                      );
                    })}
                  ...
                  <div className="flex flex-col gap-4 w-52 shrink-0 z-10 relative">
                    <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider text-center border-b border-slate-900 pb-1 mb-1 font-mono">
                    ...
                    {incomingTables.length > 2 && (
                      <div className="p-3 bg-slate-900/50 border border-dashed border-slate-800 rounded-2xl text-center">
                    ...
                    {incomingTables.length === 0 && (
                      <div className="h-28 flex flex-col items-center justify-center border border-dashed border-slate-900 rounded-2xl bg-slate-950/30">
                        <span className="text-[10px] text-slate-650 italic font-sans">No incoming links</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-4 w-52 shrink-0 z-10 relative">
                    <div className="text-[9px] text-slate-550 font-bold uppercase tracking-wider text-center border-b border-slate-900 pb-1 mb-1 font-mono">
            ...
            {/* Columns & attributes table representation */}
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
              <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800/80 pb-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="text-sm font-extrabold font-mono text-slate-800 dark:text-slate-100 uppercase tracking-wider">
                    Table Specification: {selectedTable.name}
                  </h3>
                </div>
                <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-mono px-3 py-1 rounded-xl border border-slate-200/40 dark:border-slate-700/30">
                  Persistent ORM Schema
                </span>
              </div>

              <div>
                <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-bold tracking-wider block mb-3">
                  Attributes & Fields Dictionary
                </span>
                <div className="border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                  <table className="w-full text-left border-collapse text-xs">
                    <thead>
                      <tr className="bg-slate-50/70 dark:bg-slate-950/40 border-b border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider text-[9px] font-sans">
                        <th className="p-3.5 pl-5">Field Name</th>
                        <th className="p-3.5">Data Type</th>
                        <th className="p-3.5 pr-5">Constraints</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-150 dark:divide-slate-800/70 font-mono">
                      {selectedTable.columns.map((col, idx) => {
                        const isPk = col.constraints?.includes("PRIMARY KEY");
                        const isFk = col.constraints?.includes("FOREIGN KEY");
                        return (
                          <tr key={idx} className="hover:bg-slate-50/40 dark:hover:bg-slate-800/15 transition-colors">
                            <td className="p-3.5 pl-5 font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                              {isPk ? (
                                <Key className="h-3.5 w-3.5 text-amber-500 shrink-0" title="Primary Key" />
                              ) : isFk ? (
                                <ExternalLink className="h-3.5 w-3.5 text-indigo-500 dark:text-indigo-400 shrink-0" title="Foreign Key" />
                              ) : (
                                <Hash className="h-3.5 w-3.5 text-slate-400 dark:text-slate-600 shrink-0" />
                              )}
                              <span>{col.name}</span>
                            </td>
                            <td className="p-3.5 font-mono">
                              <span className={`text-[10px] px-2 py-0.5 rounded font-semibold ${getDataTypeBadgeStyle(col.type)}`}>
                                {col.type}
                              </span>
                            </td>
              ...
              {selectedTable.relationships?.length > 0 && (
                <div className="pt-2">
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-bold tracking-wider block mb-3.5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {selectedTable.relationships.map((rel, idx) => {
                      ...
                      return (
                        <div 
                          key={idx} 
                          className="bg-slate-50/50 dark:bg-slate-900/50 p-4.5 rounded-2xl border border-slate-200 dark:border-slate-800/80 flex flex-col justify-between hover:shadow-md transition-all group"
                        >
                          <div className="flex items-center justify-between mb-3.5">
                            ...
                            <span className={`text-[9px] font-mono px-2.5 py-0.5 rounded-full font-bold ${
                              rel.type === 'one-to-many' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-400' :
                              rel.type === 'many-to-one' ? 'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400' :
                              rel.type === 'one-to-one' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' :
                          ...
                          <div className="space-y-3.5">
                            <div className="flex items-center gap-2.5 text-xs text-left">
                              ...
                              <code className="font-mono font-bold bg-white dark:bg-slate-950 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200">
                                {rel.foreignKey}
                              </code>
                            </div>
                            
                            <div className="flex items-center justify-between pt-2.5 border-t border-slate-150 dark:border-slate-800/50">
                              <div className="flex items-center gap-2 text-xs font-mono font-bold">
                                <span className="text-slate-600 dark:text-slate-400">{selectedTable.name}</span>
                                <ArrowRight className="h-3.5 w-3.5 text-slate-400" />
                                <span className="bg-indigo-600 dark:bg-indigo-500/20 text-white dark:text-indigo-300 px-2 py-0.5 rounded font-mono text-[10px]">
                                  {rel.targetTable}
                                </span>
                              </div>
                              ...
                              {targetTableObj && (
                                <button
                                  ...
                                  className="flex items-center gap-0.5 text-[10px] text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 font-semibold cursor-pointer group-hover:translate-x-0.5 transition-transform"
                                >
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm lg:col-span-2 space-y-6 text-left">
            ...
            <div>
              <span className="text-[10px] text-slate-450 dark:text-slate-500 uppercase font-bold tracking-wider block mb-3.5">
                ...
                {activeProject.database?.tables?.map((table, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      onClick={() => setSelectedTable(table)}
                      className="bg-slate-50/50 dark:bg-slate-900/30 p-4.5 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-indigo-400 dark:hover:border-indigo-800 hover:shadow-md cursor-pointer transition-all group text-left"
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2 overflow-hidden">
                          <Database className="h-4 w-4 text-indigo-500 dark:text-indigo-400 group-hover:scale-110 transition-transform shrink-0" />
                          <span className="text-xs font-mono font-bold text-slate-800 dark:text-slate-200 truncate">
                            {table.name}
                          </span>
                        </div>
                        <span className="bg-slate-200/50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-[9px] px-2 py-0.5 rounded font-mono font-semibold shrink-0">
                          {table.columns.length} columns
                        </span>
                      </div>
                      
                      <div className="space-y-2 mt-3.5 text-[10px] text-slate-550 dark:text-slate-400">
                        {pkCol && (
                          <div className="flex items-center gap-1.5 font-mono">
                            <Key className="h-3.5 w-3.5 text-amber-500 shrink-0" />
                            <span className="truncate">PK: <code className="bg-white dark:bg-slate-950 px-1.5 py-0.2 rounded border dark:border-slate-800 text-slate-700 dark:text-slate-350">{pkCol.name}</code></span>
                          </div>
                        )}
                        
                        <div className="flex items-center justify-between border-t border-slate-150 dark:border-slate-800/50 pt-2 text-[10px]">
