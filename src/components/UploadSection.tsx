...
export default function UploadSection({ onAnalysisCompleted }: UploadSectionProps) {
  ...
  const getPlanPrice = (plan: 'basic' | 'super') => {
    if (plan === 'basic') return 1.0;
    const baseCount = 6;
    if (selectedFilesToAudit.length > baseCount) {
      return 5.0 + (selectedFilesToAudit.length - baseCount) * 0.10;
    }
    return 5.0;
  };

  const loadZipFileStructure = async (zipFile: File) => {
    try {
      setIsExtractingZip(true);
      const zip = new JSZip();
      const contents = await zip.loadAsync(zipFile);
      const fileNames = Object.keys(contents.files).filter(filename => {
        ...
        if (ext && EXCLUDED_EXTENSIONS.includes(ext)) return false;
      ...
      setZipFilesList(fileNames);
      setSelectedFilesToAudit(fileNames.slice(0, 6));
    } catch (err) {
      console.error("Error reading zip file content list:", err);
    } finally {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      ...
      if (droppedFile.name.endsWith('.zip')) {
        setFile(droppedFile);
        setErrorMessage('');
        loadZipFileStructure(droppedFile);
      } else {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (selectedFile.name.endsWith('.zip')) {
        ...
        setErrorMessage('');
        loadZipFileStructure(selectedFile);
      } else {
        setErrorMessage('Niepoprawny format pliku. Prześlij archiwum ZIP (.zip) zawierające kod źródłowy.');
      }
    }
  ...
  const startAnalysis = async (planToUse?: 'basic' | 'super') => {
    ...
    if (plan === 'super' && selectedFilesToAudit.length > 0) {
      formData.append('selectedFiles', JSON.stringify(selectedFilesToAudit));
    }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-slate-900/60 backdrop-blur-md p-6 rounded-2xl border border-slate-800 shadow-xl max-w-3xl mx-auto">
        <h2 className="text-xl font-bold text-slate-100 tracking-tight text-center flex items-center justify-center gap-2">
          <Shield className="text-indigo-400 fill-indigo-500/20" size={20} />
        ...
        {status === 'idle' ? (
          <div className="mt-6 space-y-5">
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${
                dragActive
                  ...
                  : 'border-slate-700 hover:border-indigo-400/50 hover:bg-slate-800/40'
              }`}
            >
              <input
                ref={inputRef}
              ...
              <div className="flex flex-col items-center">
                <UploadCloud size={44} className="text-slate-500 mb-3" />
            ...
            {file && (
              <div className="space-y-4 text-left animate-in fade-in duration-300">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div 
                      ...
                      className={`relative p-5 rounded-2xl border cursor-pointer transition-all duration-300 ${
                        selectedPlan === 'basic' 
                          ? 'bg-slate-800/60 border-slate-600 shadow-md ring-1 ring-slate-600' 
                          : 'bg-slate-900/40 border-slate-800 hover:border-slate-600 hover:bg-slate-800/40'
                      }`}
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="text-sm font-bold text-slate-200">Skan Podstawowy</h4>
                          <p className="text-[10px] text-slate-400 mt-0.5">Szybka weryfikacja antywzorców i OWASP</p>
                        </div>
                        <span className="text-sm font-black text-slate-300 bg-slate-800 px-2.5 py-1 rounded-xl border border-slate-700">1 PLN</span>
                      </div>
                      <ul className="mt-4 space-y-1.5 text-xs text-slate-400">
                        <li className="flex items-center gap-1.5">
                          <Check size={12} className="text-emerald-400" /> Skaner heurystyczny regex
                        </li>
                        <li className="flex items-center gap-1.5">
                          <Check size={12} className="text-emerald-400" /> Klasyczne luki i wycieki kluczy
                        </li>
                    ...
                    <div 
                      ...
                      className={`relative p-5 rounded-2xl border cursor-pointer transition-all duration-300 ${
                        selectedPlan === 'super' 
                          ...
                          : 'bg-slate-900/40 border-slate-800 hover:border-indigo-500/30 hover:bg-indigo-950/20'
                      }`}
                    >
                      {/* Highlight Badge */}
                      <div className="absolute -top-3 right-4 bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-[8px] font-extrabold px-2.5 py-0.5 rounded-full shadow-md flex items-center gap-1 border border-indigo-400/30">
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="text-sm font-bold text-indigo-300 flex items-center gap-1">
                            Super Skan AI <Zap size={12} className="text-amber-400 fill-amber-400/20 animate-pulse" />
                          </h4>
                          <p className="text-[10px] text-indigo-400/80 mt-0.5">Heurystyki + Modelowanie Gemini 3.5 Flash</p>
                        </div>
                        <span className="text-sm font-black text-white bg-indigo-600 px-2.5 py-1 rounded-xl shadow-md">{getPlanPrice('super').toFixed(2).replace('.', ',')} PLN</span>
                      </div>
                      <ul className="mt-4 space-y-1.5 text-xs text-slate-300">
                        <li className="flex items-center gap-1.5 font-medium">
                          <Check size={12} className="text-indigo-400" /> Etap 1: Fast Heuristic Scan
                        </li>
                        <li className="flex items-center gap-1.5 font-medium">
                          <Check size={12} className="text-indigo-400" /> Etap 2: Dynamiczny audyt logiczny AI
                        </li>
                        <li className="flex items-center gap-1.5 font-medium">
                          <Check size={12} className="text-indigo-400" /> Inteligentne łaty kodu (Unified Diff)
                        </li>
                        <li className="flex items-center gap-1.5 font-medium">
                          <Check size={12} className="text-indigo-400" /> Interaktywne czatowanie z AI
                        </li>
                      </ul>
                    </div>
                  </div>
                ...
                {/* File list selector for Super AI Plan */}
                {selectedPlan === 'super' && zipFilesList.length > 0 && (
                  <div className="bg-slate-950/60 rounded-xl p-4 border border-slate-800 space-y-3 mt-4">
                    <div className="flex justify-between items-center">
                      <h4 className="text-xs font-bold text-indigo-300 uppercase tracking-wider flex items-center gap-1.5">
                        <Sparkles size={12} className="text-indigo-400" /> Wybierz pliki do analizy AI:
                      </h4>
                      <span className="text-[10px] bg-slate-850 text-indigo-300 font-semibold px-2 py-0.5 rounded border border-slate-700">
                        Zaznaczono: {selectedFilesToAudit.length} {selectedFilesToAudit.length > 6 ? `(+${selectedFilesToAudit.length - 6} płatnych)` : ''}
                      </span>
                    </div>
                    {selectedFilesToAudit.length > 6 && (
                      <p className="text-[10px] text-amber-400 font-medium">
                        Info: Zaznaczono ponad 6 plików. Każdy kolejny plik dodaje 0,10 PLN do ceny analizy.
                      </p>
                    )}
                    ...
                    <div className="max-h-48 overflow-y-auto border border-slate-800/80 rounded-lg divide-y divide-slate-900/60 bg-slate-900/20 scrollbar-thin">
                      {zipFilesList.map((filename) => {
                        const isChecked = selectedFilesToAudit.includes(filename);
                        return (
                          <label key={filename} className="flex items-center gap-2.5 px-3 py-1.5 text-xs text-slate-300 hover:bg-slate-800/40 cursor-pointer transition-colors">
                            <input
                              type="checkbox"
                              checked={isChecked}
                              onChange={(e) => {
            ...
            {errorMessage && (
              <div className="flex items-start gap-2 p-3 bg-rose-500/10 border border-rose-500/20 rounded-lg text-rose-400 text-xs text-left animate-shake">
              ...
              {file && (
                <>
                  <button
                    ...
                    className="px-4 py-2 border border-slate-700 text-slate-400 rounded-lg text-xs font-semibold hover:bg-slate-800 transition-colors"
                  >
                    Wyczyść plik
                  </button>
                  <button
                    onClick={handleStartClicked}
                    className="px-5 py-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white rounded-lg text-xs font-bold transition-all shadow-md flex items-center gap-1.5 border border-indigo-500/30"
                  >
                    <CreditCard size={14} />
          ...
          <div className="mt-6 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-slate-900/60 rounded-xl p-3 border border-slate-800 text-center shadow-inner">
                <span className="text-[10px] text-slate-500 font-semibold uppercase">Przeskanowane pliki</span>
                <div className="text-lg font-bold text-slate-200 mt-1">{scannedFilesCount}</div>
              </div>

              <div className="bg-rose-950/30 rounded-xl p-3 border border-rose-500/20 text-center shadow-inner">
                <span className="text-[10px] text-rose-500 font-semibold uppercase">Luki bezpieczeństwa</span>
                <div className="text-lg font-bold text-rose-400 mt-1">{foundIssues.security}</div>
              </div>

              <div className="bg-amber-950/30 rounded-xl p-3 border border-amber-500/20 text-center shadow-inner">
                <span className="text-[10px] text-amber-500 font-semibold uppercase">Code Smells</span>
                <div className="text-lg font-bold text-amber-400 mt-1">{foundIssues.quality}</div>
              </div>

              <div className="bg-indigo-950/30 rounded-xl p-3 border border-indigo-500/20 text-center shadow-inner">
                <span className="text-[10px] text-indigo-400 font-semibold uppercase">Refaktoryzacje</span>
                <div className="text-lg font-bold text-indigo-300 mt-1">{foundIssues.refactor}</div>
              </div>
            </div>
            ...
            <div className="flex justify-center pt-2">
              <button
                onClick={cancelOrReset}
                className="px-5 py-2 border border-slate-700 hover:bg-slate-800 text-slate-400 rounded-lg text-xs font-semibold transition-colors shadow-sm"
              >
                Przerwij i wyjdź
              </button>
            </div>
            ...
            <div className="p-6 bg-slate-50/50 border-b border-slate-100 flex justify-between items-center text-xs">
              ...
              <div className="text-right">
                ...
                <span className="text-base font-black text-slate-900">
                  {getPlanPrice(selectedPlan).toFixed(2).replace('.', ',')} PLN
                </span>
              </div>
            </div>
              ...
              <div className="space-y-2 pt-2">
                ...
                {paymentMethod === 'blik' && (
                  <button 
                    ...
                    className="w-full py-1.5 border border-dashed border-pink-200 hover:border-pink-400 text-pink-600 hover:text-pink-700 bg-pink-50/20 text-[10px] font-bold rounded-lg transition-colors flex items-center justify-center gap-1"
                  >
                ...
                {isProcessingPayment ? (
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex gap-2">
                      ...
                      >
                        <Lock size={12} /> Autoryzuj {getPlanPrice(selectedPlan).toFixed(2).replace('.', ',')} PLN
                      </button>
                    </div>
