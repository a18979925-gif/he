...
// Load environment variables before instantiating the SDK
dotenv.config();
...
import { scanPhp } from './rules/php';
import { scanCSharp } from './rules/csharp';
import { scanRuby } from './rules/ruby';
import { scanKotlin } from './rules/kotlin';
import { scanSwift } from './rules/swift';
import { scanSql } from './rules/sql';
...
function scanHeuristics(filePath: string, content: string, language: string): AuditIssue[] {
  ...
  lines.forEach((lineText, index) => {
    ...
    if (['TypeScript', 'TypeScript (JSX)', 'JavaScript', 'JavaScript (JSX)'].includes(language)) {
      ...
      issues.push(...scanPhp(filePath, lineText, lineNum));
    } else if (language === 'C#') {
      issues.push(...scanCSharp(filePath, lineText, lineNum));
    } else if (language === 'Ruby') {
      issues.push(...scanRuby(filePath, lineText, lineNum));
    } else if (language === 'Kotlin') {
      issues.push(...scanKotlin(filePath, lineText, lineNum));
    } else if (language === 'Swift') {
      issues.push(...scanSwift(filePath, lineText, lineNum));
    } else if (language === 'SQL') {
      issues.push(...scanSql(filePath, lineText, lineNum));
...
export async function runAnalysis(analysisId: string, zipBuffer: Buffer, fileName: string, plan: 'basic' | 'super' = 'basic', selectedFilesToAudit?: string[]) {
  try {
    // 1. EXTRACT STAGE
    ...
    const parsedFiles: { path: string; name: string; language: string; size: number; content: string; metrics?: FileMetrics }[] = [];
    ...
    for (const entry of zipEntries) {
      ...
      try {
        ...
        parsedFiles.push({
          path: entryName,
          name: entryName, // Need name for processAnalysis
          language,
          size,
          content,
          metrics,
    ...
    // We no longer loop file-by-file for heuristics and streaming, 
    // because processAnalysis does it all at once.
    // We will simulate a quick scan progress update.
    broadcastEvent(analysisId, {
      type: 'STAGE_CHANGED',
      status: 'scanning_heuristics',
      progress: 50,
      message: `Deep analyzing static ASTs and building architecture graphs...`,
    });
    
    summary.scannedFileCount = parsedFiles.length;
    ...
    const aiClient = plan === 'super' ? ai : null;
    const realAnalysis = await processAnalysis(parsedFiles, fileName, aiClient, selectedFilesToAudit);

    // Update summary metrics from the new analysis
    const allSecurity = realAnalysis.security || [];
    const allPerf = realAnalysis.performance || [];
    const allRefactor = realAnalysis.refactoring || [];
    const allBugs = realAnalysis.healthReasons?.filter(r => r.category.toLowerCase().includes('bug')) || [];

    summary.issuesCount.security = allSecurity.length;
    summary.issuesCount.quality = allPerf.length + allBugs.length;
    summary.issuesCount.refactor = allRefactor.length;
    summary.criticalCount = allSecurity.filter(s => s.severity?.toLowerCase() === 'critical').length;

    // Stream issues out for compatibility (mapping to old format quickly)
    for (const sec of allSecurity) {
       broadcastEvent(analysisId, { type: 'ISSUE_FOUND', issue: {
         ...
         line: sec.line,
         category: 'security',
         severity: (sec.severity?.toLowerCase() as any) || 'warning',
         title: sec.category,
         description: sec.description,
         snippet: sec.oldCode || '',
         suggestion: sec.solution || sec.newCode || ''
       } });
    }
    ...
    analyses.set(analysisId, finalReport as AnalysisReport);
    ...
    broadcastEvent(analysisId, {
      type: 'ANALYSIS_DONE',
      report: finalReport as AnalysisReport,
    });
